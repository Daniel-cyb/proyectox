...
export default function MentionsLineChart({ data }) {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleTime()
      .domain([d3.min(mentionsByDate, d => d.date), d3.max(mentionsByDate, d => d.date)])
      .range([0, width]);
    ...
    svg.append('g')
      ...
      .call(d3.axisBottom(x).ticks(8).tickSizeOuter(0))  // Ajuste de ticks
      .selectAll("text")  // Ajustar tamaño del texto en el eje X
      .style("font-size", "12px")
      .attr("dy", "0.75em");

    // Eje Y (Conteo de menciones)
    svg.append('g')
      .call(d3.axisLeft(y).ticks(10))  // Ajuste de ticks para el eje Y
      .selectAll("text")  // Ajustar tamaño del texto en el eje Y
      .style("font-size", "12px");
    ...
    svg.selectAll('.dot')
      ...
      .attr('cx', d => x(d.date))
      .attr('cy', d => y(d.count))
      .attr('r', 4)  // Tamaño del radio de los puntos
      .attr('fill', '#4a90e2')
      .attr('stroke', '#fff')
    ...
    svg.selectAll('.label')
      ...
      .attr('fill', 'black')
      .style('font-size', '10px')  // Ajuste de tamaño del texto para las etiquetas
