...
// Definición del tipo para cada ruta
interface Route {
  href: string;
  label: string;
    ...
    routes.forEach((route) => {
      if (route.subRoutes) {
        route.subRoutes.forEach((subRoute) => {
          if (pathname && pathname.startsWith(subRoute.href)) {
            ...
            {routes.map((route) => (
              <div key={route.href}>
                {route.subRoutes ? (
                      ...
                      <div className="pl-10 space-y-1">
                        {route.subRoutes.map((subRoute) => (
                          <Link
                            key={subRoute.href}
                            href={subRoute.href}
                            className="text-sm p-2 block text-zinc-400 hover:text-white hover:bg-white/10 rounded-lg transition"
                          >
                            {subRoute.label}
                          </Link>
                        ))}
                  ...
                  <Link
                    href={route.href}
                    className="text-sm flex p-3 w-full text-zinc-400 hover:text-white hover:bg-white/10 rounded-lg transition"
                  >
                    <route.icon className="h-5 w-5 text-sky-500 mr-3" />
                    {isSidebarOpen && route.label}
                  </Link>
