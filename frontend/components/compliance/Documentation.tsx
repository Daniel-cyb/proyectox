...
export default function Documentation() {
  const [documents, setDocuments] = useState([]); // Lista de documentos cargados
  ...
  const handleSelectDocument = (file) => {
    setSelectedDocument(`http://localhost:3052/uploads/${file}`); // Asignar la ruta del archivo seleccionado
  };
  ...
  return (
    <div className="container mx-auto p-6">
        ...
        <div className="border p-4">
          ...
          <ul>
            {documents.map((file, index) => (
              <li
                key={index}
                className={`p-2 cursor-pointer ${selectedDocument === `http://localhost:3052/uploads/${file}` ? 'bg-blue-200' : ''}`}
        ...
        <div className="border p-4 col-span-2">
          {selectedDocument ? (
            <div>
              ...
              ></iframe> {/* Vista previa del documento PDF */}
              <a
            ...
            <p className="text-gray-500">Selecciona un documento de la lista para previsualizarlo.</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-6">
        <label className="block mb-2 text-lg font-medium text-gray-700">Cargar Documento</label>
        <input
          type="file"
        ...
        {isUploading && (
          <div className="mt-4">
            <label className="block text-sm font-medium text-gray-700">Progreso de carga:</label>
